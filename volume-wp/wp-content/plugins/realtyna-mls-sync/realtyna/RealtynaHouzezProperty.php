    ...
    public function import( $slugValues , $demo = true ){
        ...
        if ( $this->mapValues( $slugValues ) !== false){
            ...
            if ( $this->propertyExists() ){
                ...
                if ( $this->isAllowedProperty() ){
                                
                    $importResult = ( $this->updateProperty( $this->getPropertyIdByIdxIdentity() , $demo ) !== false );

                    if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ){

                        error_log( 'import -> Exists -> isAllowedProperty -> update : ' . $importResult );
                        
                    }

                }else{

                    $importResult = $this->deleteProperty( $this->getPropertyIdByIdxIdentity() ) ;

                    if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ){

                        error_log( 'import -> Exists -> Not isAllowedProperty -> delete : ' . $importResult );
                        
                    }

                }                

            }elseif( $this->isAllowedProperty() ){
    
                $importResult = ( $this->insertProperty( $demo ) !== false );

                if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ){

                    error_log( 'import -> Not Exists -> isAllowedProperty -> Insert : ' . $importResult );
                ...
                if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ){
                    ...
                    error_log( 'import -> Not Exists -> Not isAllowedProperty : ' . var_export( $slugValues , true ) );
                    error_log( 'MappedFields : ' . var_export( $this->customFields , true ) );
